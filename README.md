...
- 游戏详情：原版链接与汉化链接一键开玩（点击计数）、点赞、收藏、热度分
- 评论：3-500 字，每分钟最多 5 条，可编辑 / 删除自己的评论
- 排行榜：热度分 = 点赞×2 + 收藏×3 + 评论×4 + 游玩数，实时更新
- 存档银行：玩家以 .txt 文件或剪贴板上传存档，管理员审核后公开，可下载计数
- 举报：辱骂 / 广告 / 色情 / 剧透 / 其他，附补充说明
- 公告栏：置顶公告 + 过期时间
- 账号与安全：登录 IP 记录、邮箱 / 密码 / 昵称每月可改一次、邮箱验证、找回密码
- 主题切换：亮色 / 暗色 / 跟随系统
- 个人中心：我的收藏 / 点赞 / 评论 / 存档 / 举报记录

### 管理后台（独立站点，管理员 = 用户名 Winster / winster）
- 仪表盘：今日访问人数（独立 IP）、PV、昨日对比、近 7 天柱状图、热度 TOP10
- 游戏管理：增删改，图片可传 URL 或本地上传（≤5MB，存数据库），可开启/关闭存档银行
- 评论管理：修改 / 删除任意评论
- 用户管理：封禁（按小时或永久）与解封、设置 / 撤销管理员
- 举报审核：确认（可选封禁时长 / 永久封禁 / 同时删除评论）或驳回，处理备注回传给举报人
- 存档审核：查看完整内容、通过 / 驳回（附原因）
- 公告管理：发布 / 编辑 / 删除，置顶与过期时间
- 审计日志：管理员全部操作留痕
