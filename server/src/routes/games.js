...
function gameSelect(userId) {
  return `
    SELECT g.*, ${SCORE_SQL} AS score,
      ${userId ? `EXISTS(SELECT 1 FROM likes l WHERE l.game_id = g.id AND l.user_id = ${userId}) AS is_liked,
      EXISTS(SELECT 1 FROM favorites f WHERE f.game_id = g.id AND f.user_id = ${userId}) AS is_favorited` : 'FALSE AS is_liked, FALSE AS is_favorited'}
    FROM games g`;
}
